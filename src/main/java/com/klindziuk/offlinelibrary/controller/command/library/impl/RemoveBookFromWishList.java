...
public class RemoveBookFromWishList implements Command {
	private static final Logger logger = Logger.getLogger(RemoveBookFromWishList.class);
	private static final String SUCCESSFUL_REMOVE_MESSAGE = "Book succefully removed from your wishlist.";
	...
	public String execute(String request) {
		...
		try {
			...
			if (libraryService.removeBookFromWishList(parameters[0], parameters[1])) {
				response = SUCCESSFUL_REMOVE_MESSAGE;
			}
